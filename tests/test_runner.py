...
def test_runner_records_failure_without_fallback(tmp_path: Path) -> None:
    sample = RoutingSample(sample_id="s1", family_id="f1", input="查天气",
        expected=RouteLabel.SEARCH, acceptable=[RouteLabel.SEARCH], risk="low",
        difficulty="clear", rationale="test", source="synthetic", split=Split.DEV)
    manifest = RunManifest(run_id="r1", provider="jev", model_version="test",
        run_mode=RunMode.LIVE_EVALUATION, split=Split.DEV, dataset_path="data",
        dataset_sha256="hash", git_commit="commit", created_at=datetime.now(UTC))
